...
class DemandePret
{
    ...
    // Lire les demandes de prêt par plage de dates
    public function searchDemandesByDateRange($date_debut, $date_fin, $matricule)
    {
        $sql = "SELECT * FROM demande_pret WHERE date_creat BETWEEN ? AND ? AND matricule = ?";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$date_debut, $date_fin, $matricule]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
}
