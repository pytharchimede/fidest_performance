...
$demandeAvanceObj = new DemandeAvance();
...
// Récupérer toutes les demandes d'avances
$demandes = $demandeAvanceObj->lireDemandesAvances();

// Filtrer les demandes d'avances selon les critères fournis
    ...
    <?php if ($nbDemandes > 0): ?>
        <?php foreach ($demandes as $demande): ?>
            <li class="list-group-item">
                <h5><i class="fas fa-credit-card"></i> <?= htmlspecialchars($demande['motif']); ?></h5>
                <p><i class="fas fa-user"></i> Nom : <?= htmlspecialchars($demande['nom']); ?></p>
                <p><i class="fas fa-id-badge"></i> Matricule : <?= htmlspecialchars($demande['matricule']); ?></p>
                <p><i class="fas fa-briefcase"></i> Fonction : <?= htmlspecialchars($demande['fonction']); ?></p>
                <p><i class="fas fa-building"></i> Service : <?= htmlspecialchars($demande['service']); ?></p>
                <p><i class="fas fa-money-bill-alt"></i> Montant demandé : <?= htmlspecialchars($demande['montant']); ?> FCFA</p>
                ...
                <div class="task-actions">
                    <!-- Bouton Exporter PDF -->
                    <a style="margin-top:19px;" href="request/export_demande_avance.php?id=<?= htmlspecialchars($demande['id']); ?>" class="btn btn-export">
                        <i class="fas fa-file-pdf"></i> Exporter
                    </a>

                    <!-- Bouton Approuver -->
                    <form method="post" action="request/traiter_demande_avance.php" class="d-inline">
